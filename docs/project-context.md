...
`npm run dev` serves the frontend and same-origin API at `http://localhost:5173`. Production requires `npm run build` and `npm start` in a Node service with HTTPS `APP_ORIGIN`, a private `AUTH_SESSION_SECRET`, and Firebase Admin credentials. A static `dist/` upload alone cannot support login. Local code validation and a Git push are not evidence of public deployment.
