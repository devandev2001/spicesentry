...
## SpiceSentry project policy

This project uses the checked-in `npm run context:refresh` wrapper for builds and
`npm run context:check` for freshness. It merges reviewed semantic facts with local
AST extraction and enforces a source allowlist. Do not run external semantic APIs,
even if provider keys are present. Do not index credentials, business rows, backups,
or `whatsapp-cron/Code.gs`. Do not install global configuration or git hooks. These
project restrictions take precedence over the upstream generic pipeline below.
Use `docs/project-context.md` and `docs/context-graph.json` for durable decisions;
verify graph findings in source. Upstream source: Graphify-Labs/graphify, graphifyy
0.8.36. The remainder of this skill and its references are bundled upstream docs.
